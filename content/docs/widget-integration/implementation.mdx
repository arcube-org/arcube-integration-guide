...
description: Technical steps for Widget Integration — authentication, ancillary selection, payment, and order confirmation.
---
...
<img src="/images/ArcubeiFrame-Widget-Drawio.png" alt="Widget Integration sequence diagram" style={{width: '100%'}} />

<small>Figure: Example process and required calls for integrating with Arcube using the widget integration method.</small>
...
This data is passed to the **tenant's back-end**, which must call the [Ingest Passenger Data](/docs/reference/essential-api-calls#61-ingest-passenger-data) API.
...
- Uses the token to make all subsequent calls to Arcube's backend.
...
## 5. Tenant Confirms Payment to Arcube
