...
description: Technical setup for Email Integration — data transfer, email authentication, and customization.
---
...
This is done using the [Passenger Data API](/docs/reference/essential-api-calls#61-ingest-passenger-data).
...
## Sending Emails on Behalf of the Airline
...
<Callout type="info" title="Action">
  Arcube will provide the correct sending IPs or domain. The tenant must update DNS settings accordingly.
</Callout>
...
<Callout type="info" title="Action">
  Arcube provides a **DKIM public key**, which the tenant adds to their domain's DNS settings.
</Callout>
...
<Callout type="info" title="Action">
  Tenant and Arcube collaborate to whitelist IPs in the email server or filtering system.
</Callout>
...
<Callout type="info" title="Action">
  Arcube and the tenant perform joint testing of email deliverability.
</Callout>
