...
description: Technical setup for the White-Label POS - data transfer, rendering, payment handling, and confirmation.
---
...
This is done via a **secure POST request**, following the same process as described in the [Widget Integration](/docs/widget-integration/implementation#1-collect-passenger-contextual-data) section.
...
- When the passenger changes display currency, the site calls [Reprice Offers and Cart](/docs/reference/essential-api-calls#63-reprice-offers-and-cart) - the offer list is re-rendered from `updatedPrices` and the cart replaced with `updatedCart`
- Before charging, the white-label site calls [Verify Prices](/docs/reference/essential-api-calls#64-verify-prices) to confirm the cart still matches the authoritative bundle - the passenger sees the updated total if any item has drifted
