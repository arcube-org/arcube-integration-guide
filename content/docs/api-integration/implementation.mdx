...
description: Technical steps for direct API integration - data ingestion, recommendations, and order confirmation.
---
...
<img src="/images/API-Integration-DrawingIO.png" alt="API Integration sequence diagram" style={{width: '100%'}} />

<small>Figure: Example process and required calls for integrating with Arcube using the direct integration method.</small>
...
→ Refer to [Ingest Passenger Data](/docs/reference/essential-api-calls#61-ingest-passenger-data) for request details.
...
→ See [Serve Recommendations](/docs/reference/essential-api-calls#62-serve-recommendations).

Arcube responds with a list of personalized ancillary products, tailored by destination, timing, and passenger profile.

If the passenger changes their display currency, call [Reprice Offers and Cart](/docs/reference/essential-api-calls#63-reprice-offers-and-cart) rather than searching again. Re-render the offer list from `updatedPrices` and replace the cart with `updatedCart`.
...
- Immediately before charging the customer, the tenant should call [Verify Prices](/docs/reference/essential-api-calls#64-verify-prices) to confirm that the cart prices still match Arcube's authoritative bundle and re-confirm with the user if any item has drifted.
...
→ Refer to [Confirm Order](/docs/reference/essential-api-calls#65-confirm-order) for details on how to complete this step.
