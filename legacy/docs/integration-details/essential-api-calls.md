...
All calls require a valid **Bearer** token obtained from the authentication endpoint. The required credentials are **client_id** and **client_secret**, which will be provided by Arcube. See: [Authentication](authentication.md).
...
!!! info "Error Handling"
    All endpoints follow the unified structure described in [Error Model](error-model.md).
